...
- 지역 또는 열차 체크박스를 눌러도 필터 영역이 자동으로 접히지 않음
- 사용자가 필터 제목을 눌렀을 때만 접기/펼치기
- 여러 지역과 여러 차종을 연속으로 선택 가능
- 전체 선택/전체 해제 시에도 필터 펼침 상태 유지
- 상단 `검색` 버튼 또는 Enter로 새 검색하면 지역/열차 제외 필터가 모두 초기화됨
- 열차 필터 체크 시에는 현재 검색 조건에서만 즉시 재계산
